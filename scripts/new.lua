...
function _generate_package_from_github(reponame)
	...
	local repoinfo = os.iorunv(
		gh.program,
		{ "repo", "view", reponame, "--json", "description,homepageUrl,licenseInfo,url,sshUrl,name,latestRelease" }
	)
	if repoinfo then
	...
	if type(latest_release) == "table" then
		...
		local version = try({
			function()
				return os.iorunv("git", { "log", "-1", "--date=format:%Y.%m.%d", "--format=%ad" }, { curdir = repodir })
			end,
		})
		if version then
